...
- Advancing a horizontal pager (e.g. `/onboarding` is a `pagingEnabled` FlatList): clicking the "Continuer" CTA does NOT trigger `scrollToIndex` on web, and `el.scrollTo({left})` snaps back (CSS scroll-snap). What works: find the scroller (`scrollWidth > clientWidth`, overflowX auto/scroll, largest), set `el.style.scrollSnapType="none"`, then assign `el.scrollLeft = slideWidth * index` (slideWidth = logical viewport width, e.g. 450). Verify by reading back `el.scrollLeft`.
