  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard title={d.users} value={statsLoading ? "—" : stats?.totalUsers ?? 0} icon={Users} color="#3b82f6" />
        <StatCard title={d.barbers} value={statsLoading ? "—" : stats?.totalBarbers ?? 0} icon={Scissors} color="hsl(var(--primary))" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 rounded-xl border bg-card p-5">
          <h2 className="text-sm font-semibold mb-4">{d.monthlyTrends}</h2>
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={monthly ?? []} barSize={8} barGap={4}>
              ...
              <Bar dataKey="reservations" name={d.reservationsLabel} fill="hsl(var(--primary))" radius={[3, 3, 0, 0]} />
              <Bar dataKey="newBarbers" name={d.newBarbers} fill="#3b82f6" radius={[3, 3, 0, 0]} />
              <Bar dataKey="newUsers" name={d.newClients} fill="#8b5cf6" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>

        <div className="rounded-xl border bg-card p-5">
          <h2 className="text-sm font-semibold mb-4">{d.topBarbers}</h2>
            ...
            {(topBarbers ?? []).slice(0, 5).map((b: any, i: number) => (
              <div key={b.barberId} className="flex items-center gap-3">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xs text-muted-foreground">{d.resShort}</p>
                </div>
              </div>
            ))}
      ...
      <div className="mt-6 rounded-xl border bg-card p-5">
        <h2 className="text-sm font-semibold mb-4">{d.recentActivity}</h2>
        <div className="space-y-2">
          ...
          {(!activity || activity.length === 0) && (
            <p className="text-xs text-muted-foreground text-center py-4">{d.noActivity}</p>
          )}
        </div>
      </div>
    </div>
